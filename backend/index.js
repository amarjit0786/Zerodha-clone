...
//     try {
//         // Using `Promise.all` to wait for all save operations to complete
...
//             await newHolding.save();  // Wait for each save to complete
...
//         res.status(500).send("Error adding holdings data.");
//     }
// });
...
//       try {
//         await Promise.all(tempPositions.map(async (item)=>{
...
//       } catch (error) {
//         console.error("Error saving holdings:", error);
//         res.status(500).send("Error adding positions data.");
//       }
// } );
